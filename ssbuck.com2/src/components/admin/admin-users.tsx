...
import { Baby, CheckCircle, Eye, Loader2, Search, Trash2, XCircle, Save } from 'lucide-react'
    ...
    <>
      <Card>
        <CardHeader>
          ...
          <CardDescription>회원을 관리하고 삭제 및 상태 변경을 할 수 있습니다.</CardDescription>
        </CardHeader>
        <CardContent>
          <div className='space-y-4'>
            <div className='flex gap-3 flex-row justify-between items-end'>
              ...
              <div className='relative w-full sm:w-80 flex gap-2'>
                ...
                <Input placeholder='사용자, 이메일 검색' className='pl-9 h-11' />
              </div>
                    ...
                    <TableBody>
                      {users.length > 0 ? (
                        users.map((user, index) => (
                          <TableRow key={user._id} className='text-muted-foreground'>
      ...
      {/* 사용자 상세 다이얼로그 */}
      <DialogUserDetail open={detailOpen} onOpenChange={setDetailOpen} user={selectedUser} />

      {/* 사용자 삭제 다이얼로그 */}
      <DialogUserDelete open={deleteOpen} onOpenChange={setDeleteOpen} onConfirm={confirmDelete} />
