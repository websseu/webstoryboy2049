...
import { generateVerificationCode, generateUsername } from '../utils'
...
export async function completeSignup(data: IUserSignUpInput) {
  try {
    ...
    return {
      success: true,
      message: `${user.name}님, 회원가입이 완료되었습니다! 사용자명: @${user.username}`,
    }
  } catch (error) {
...
// 기존 사용자들에게 username 추가
export async function addUsernameToExistingUsers() {
  try {
    await connectToDatabase()

    // username이 없는 사용자들 찾기
    ...
    for (const user of usersWithoutUsername) {
      if (user.name) {
        ...
        console.log(`사용자 ${user.name}에게 username ${username} 추가됨`)
      }
    }

    return {
      success: true,
      message: `${updatedCount}명의 사용자에게 username이 추가되었습니다.`,
      updatedCount,
    }
  } catch (error) {
    console.error('기존 사용자 username 추가 오류:', error)
    return {
      success: false,
      message: '기존 사용자 username 추가 중 오류가 발생했습니다.',
    }
  }
}
