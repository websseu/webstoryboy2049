...
import { IEmailOnlyInput, IUserInfoInput, IUserSignInInput, IVerifyEmailInput } from '../type'
import { EmailOnlySchema, UserInfoSchema, VerifyEmailSchema } from '../validator'
import bcrypt from 'bcryptjs'
import { signIn, signOut } from '@/auth'
import { redirect } from 'next/navigation'

// initiateSignup 1단계: 이메일 인증 코드 발송
// verifySignup 2단계: 인증 코드 확인
// completeSignup 3단계: 회원가입 완료
...
// 3단계: 회원가입 완료
export async function completeSignup(data: IUserInfoInput) {
  try {
    // zod 비동기 유효성 검사
    const { email, name, password } = await UserInfoSchema.parseAsync(data)
