...
import { Mail, Send, Handshake, UserPlus, CheckCircle, FileText, Shield } from 'lucide-react'
...
import { completeSignup, initiateSignup, verifySignup } from '@/lib/actions/user.action'
...
export default function DialogSignup({ open, onOpenChange, onOpenLoginDialog }: DialogSignupProps) {
  ...
  // 4단계: 사용자 정보 입력 폼
  const userInfoForm = useForm<IUserInfoInput>({
  ...
  const onSubmitUserInfo = async (data: IUserInfoInput) => {
    try {
      const completeData = {
        email: userEmail,
        name: data.name,
  ...
  const goToLogin = () => {
    ...
    resetAllForms()
    setTimeout(() => {
      if (onOpenLoginDialog) onOpenLoginDialog()
      else window.dispatchEvent(new CustomEvent('openLoginDialog'))
    }, 300)
  }
                  ...
                  render={({ field }) => (
                    <FormItem className='flex items-center justify-end gap-1'>
                      <FormControl>
                        <Checkbox
                          checked={field.value}
                          onCheckedChange={(checked) => handleAllAgreed(checked as boolean)}
                          className='mt-1'
                        />
                      </FormControl>
                      <FormLabel className='font-medium text-muted-foreground'>전체 동의</FormLabel>
                    </FormItem>
                  )}
                />
            ...
            <Form {...emailForm}>
              <form onSubmit={emailForm.handleSubmit(onSubmitEmail)}>
                <div className='text-center p-4 bg-blue-50 rounded-lg mb-2'>
                  ...
                  <p className='text-xs text-muted-foreground mt-1'>
                    이메일이 기억나지 않으면? <span className='uline'>이메일 찾기</span>
                  </p>
                </div>
              ...
              <form onSubmit={verificationForm.handleSubmit(onVerifyEmail)}>
                <div className='space-y-4'>
                  <div className='text-center p-4 bg-blue-50 rounded-lg'>
                    ...
                    <p className='text-sm'>
                      <span className='uline'>{userEmail}</span> 인증번호가 전송되었습니다.
                    </p>
                    <p className='text-xs text-muted-foreground mt-1'>
                      5분 내에 인증을 완료해주세요.
                    </p>
              ...
              <form onSubmit={userInfoForm.handleSubmit(onSubmitUserInfo)}>
                <div className='space-y-2'>
                  <div className='text-center p-4 bg-green-50 rounded-lg'>
                    ...
                    <p className='text-xs text-green-600 mt-1'>마지막 단계입니다.</p>
                  </div>
