...
export default function AdminStores() {
  ...
  const [deletedStore, setDeletedStore] = useState<IStore | null>(null)
  ...
  // 삭제하기
  const handleDeleteClick = (store: IStore) => {
    setDeleteOpen(true)
    setDeletedStore(store)
  }

  // 삭제 성공 완료 처리(부분로딩)
  const handleDeleteSuccess = (deletedStore: IStore) => {
    // 목록에서 삭제된 게시글 제거
    setStores((prevStores) =>
      prevStores.filter((store) => store._id !== deletedStore._id)
    )
