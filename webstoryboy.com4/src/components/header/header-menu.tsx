        ...
        <NavigationMenuList>
          {data.menuData.map((section) => (
            <NavigationMenuItem key={section.label}>
              ...
              <NavigationMenuContent>
                <ul className='grid gap-2 p-2 md:w-[300px] lg:w-[440px] lg:grid-cols-[.6fr_1fr]'>
                  <li className='row-span-4 h-[225px]'>
                    <NavigationMenuLink asChild>
                      <Link
                        className='flex h-full w-full select-none flex-col justify-end rounded-md bg-gradient-to-b from-muted to-muted p-4 no-underline outline-red-500'
                        href={section.card.href}
                      >
                        <div className='flex'>
                          {Array.from({ length: section.card.iconCount }).map((_, i) => (
                            <MdStars key={i} className='text-black h-5 w-5 dark:text-white' />
                          ))}
                        </div>
