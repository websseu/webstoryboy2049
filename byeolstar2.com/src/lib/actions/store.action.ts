...
// getAllStores : 모든 스토어 조회
// getStoresPaginated : 모든 스토어 조회(페이지)
// getStoreById : ID로 특정 스토어 조회
// getStoreByStoreId : storeId로 스토어 조회
// getStoresByLocation : 지역별 스토어 조회
// createStore : 새 스토어 생성
// updateStore : 스토어 정보 수정
// deleteStore : 스토어 삭제
// searchStores : 스토어 검색 (이름, 주소, 태그로)
// getStoresByTag : 태그별 스토어 조회
// getNearbyStores : 근처 스토어 조회 (위도, 경도 기반)
...
// 모든 스토어 조회(페이지)
export async function getStoresPaginated(page = 1, limit = 10, searchQuery?: string) {
  try {
    await connectToDatabase()
